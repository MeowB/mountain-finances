...
const ModalEditPot = ({ setModalIsOpen, potData }: { setModalIsOpen: React.Dispatch<React.SetStateAction<boolean>>, potData: any }) => {
  const [bulletColor, setBulletColor] = useState<string>(potData.color);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`/api/updatePot/${potData.id}`, {
        method: "PUT",
        headers: {
  ...
  return (
    <div className="modalEditPot">
      <h3>Edit Pot</h3>
      <p>Edit the details of your pot. Make sure to save your changes.</p>
